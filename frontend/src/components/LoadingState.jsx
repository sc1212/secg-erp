  ...
  return (
    <div className="flex flex-col items-center justify-center h-64 gap-4" aria-live="polite">
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="px-4 py-2 bg-brand-card border border-brand-border rounded-lg text-sm lg:hover:border-brand-gold/40 transition-colors"
        >
          Try again
        </button>
      )}
...
export function EmptyState({ title, message }) {
  return (
    <div className="flex flex-col items-center justify-center h-48 gap-2">
      ...
      {message && <div className="text-xs text-brand-muted/60">{message}</div>}
    </div>
  );
}
